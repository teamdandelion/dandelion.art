...
import { defaultParachutePodProps, Pod, type PodProps } from "./Pod";
...
export function Parachute(props: ParachuteProps) {
  ...
  const arms = Array.from({ length: props.numArms }, (_, i) => {
    ...
    const armLength = props.armLength * props.armLengthDecay ** distFromCenter;
    const angle =
    ...
    const segments = points.slice(1).map((point, index) => {
      ...
      return (
        <line
          key={`${prevPoint.x}-${prevPoint.y}-${point.x}-${point.y}`}
          x1={prevPoint.x}
          y1={prevPoint.y}
          x2={point.x}
          y2={point.y}
    ...
    return (
      <g key={`${startPoint.x}-${startPoint.y}-${angle}`}>
        {segments}
        <Pod
          {...props.podProps}
          x={endPoint.x}
